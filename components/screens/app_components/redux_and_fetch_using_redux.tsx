...
export const Login = ({navigation}: any) => {
  ...
  return (
    <View style={styles.container}>
      ...
      />
      <Text style={{fontSize: 20}}>Your age is, {age.toString()}</Text>
      <Text style={{fontSize: 30}}>
        Fetched From API using Redux {age.toString()}
      </Text>
      <FlatList
        data={cities}
        renderItem={({item}) => (
          <View style={styles.item}>
            <Text style={styles.button_text}>{item.city},</Text>
          </View>
        )}
  ...
  return (
    <View style={[styles.container, {justifyContent: 'center'}]}>
      ...
      <TextInput
        style={styles.input}
        value={age.toString()}
        onChangeText={val => dispatch(setAge(parseInt(val)))}
      />
      ...
      <Pressable
        ...
        <Text style={styles.button_text}>Increase Age</Text>
      </Pressable>
    </View>
  );
};
